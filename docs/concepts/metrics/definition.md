...
Metrics are defined in SQL files in the `metrics/` directory of your Vulcan project. A single file can contain multiple metric definitions.
...
For example, a Vulcan project might contain the `prod.users` and `prod.searches` models with the following `MODEL` DDLs.
...
Because those models specify their grain and references, Vulcan can correctly generate code for a metric that uses columns from both models.
...
Because the `prod.users.country` and `prod.searches.num_searches` models have specified their grains/references, Vulcan can automatically do the correct join between them.
