...
A `SEED` is a special kind of model in which data is sourced from a static dataset defined as a CSV file (rather than from a data source accessed via SQL or Python). The CSV files themselves are a part of your Vulcan project.

Since seeds are also models in Vulcan, they capitalize on all the same benefits that SQL or Python models provide:
...
Similar to [SQL models](./sql_models.md), `SEED` models are defined in files with the `.sql` extension in the `models/` directory of the Vulcan project.
...
```
The `path` attribute contains the path to the seed's CSV file **relative** to the path of the model's `.sql` file. If you want to specify a path relative to the root of the Vulcan project, use the `$root` marker (see [Markers](#markers)).
...
The `$root` marker can be used in the `path` attribute to indicate that the CSV file path is relative to the root of the Vulcan project:
...
Vulcan expects seed files to be encoded according to the [UTF-8](https://en.wikipedia.org/wiki/UTF-8) standard. Using a different encoding may lead to unexpected behavior.

## Example

In this example, we use the model definition from the previous section saved in the `models/national_holidays.sql` file of the Vulcan project.
...
When we run the `vulcan plan` command, the new seed model is automatically detected:
```bash hl_lines="8-9"
$ vulcan plan
======================================================================
...
You can now run a custom query against the table with `vulcan fetchdf`:
```bash
$ vulcan fetchdf "SELECT * FROM test_db.national_holidays"
...
Changes to the seed CSV file get picked up when the `vulcan plan` command is run:
```bash
$ vulcan plan
======================================================================
Successfully Ran 0 tests against duckdb
