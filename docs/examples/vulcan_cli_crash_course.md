...
This doc is designed to get you intimate with a **majority** of the Vulcan workflows you’ll use to build *and* maintain transformation data pipelines. The goal is to get Vulcan into muscle memory in 30 minutes or less.
...
You can follow along in the [open source GitHub repo](https://github.com/sungchun12/vulcan-cli-crash-course).

If you're new to how Vulcan uses virtual data environments, [watch this quick explainer](https://www.loom.com/share/216835d64b3a4d56b2e061fa4bd9ee76?sid=88b3289f-e19b-4ccc-8b88-3faf9d7c9ce3).
...
You can make changes quickly and confidently through one simple command: `vulcan plan dev`
...
Note: If you run this without making any changes, Vulcan will prompt you to make changes or use the `--include-unmodified` flag like this `vulcan plan dev --include-unmodified`. We recommend you make changes first before running this command to avoid creating a lot of noise in your dev environment with extraneous virtual layer views.
...
=== "Vulcan"
    ...
    ```bash
    vulcan plan <environment> --auto-apply
    ```

=== "Tobiko Cloud"

    ```bash
    tcloud vulcan plan dev
    ```

    ```bash
    tcloud vulcan plan <environment>
    ```

    If you want to move faster, you can add the `--auto-apply` flag to skip the manual prompt and apply the plan. You should do this when you're familiar with the plan output, and don't need to see tiny changes in the diff output before applying the plan.

    ```bash
    tcloud vulcan plan <environment> --auto-apply
    ```
...
=== "Vulcan"
    ...
    ```bash
    vulcan table_diff prod:dev vulcan_example.full_model --show-sample
    ```

    ```bash
    vulcan table_diff <environment>:<environment> <model_name> --show-sample
    ```

    ```bash
    vulcan table_diff prod:dev -m '*' --show-sample
    ```

=== "Tobiko Cloud"

    ```bash
    tcloud vulcan table_diff prod:dev vulcan_example.full_model --show-sample
    ```

    ```bash
    tcloud vulcan table_diff <environment>:<environment> <model_name> --show-sample
    ```

    ```bash
    tcloud vulcan table_diff prod:dev -m '*' --show-sample
    ```

??? "Example Output"
    I compare the `prod` and `dev` environments for `vulcan_example.full_model`.
...
1. Create and audit external models outside of Vulcan's control (ex: data loaded in by Fivetran, Airbyte, etc.)
...
Sometimes models `SELECT` from tables/views that are outside of Vulcan's control. Vulcan can automatically parse their fully qualified names from model definitions (ex: `bigquery-public-data`.`ga4_obfuscated_sample_ecommerce`.`events_20210131`) and determine their full schemas and column data types.

These "external model" schemas are used for column level lineage. You can also add audits to test data quality. If an audit fails, Vulcan prevents downstream models from wastefully running.
...
=== "Tobiko Cloud"
    ...
    ```bash
    tcloud vulcan create_external_models
    ```
...
??? "Example Output"
    ...
    `vulcan create_external_models` output file:
...
=== "Vulcan"
    ...
    Create a unit test based on 5 rows from the upstream `vulcan_example.incremental_model`.

    ```bash
    vulcan create_test vulcan_example.full_model \
...
=== "Tobiko Cloud"
    ...
    ```bash
    tcloud vulcan create_test demo.stg_payments \
      --query demo.seed_raw_payments \
      "select * from demo.seed_raw_payments limit 5"
    ```

    ```bash
    tcloud vulcan create_test <model_name> \
      --query <model_name upstream> \
      "select * from <model_name upstream> limit 5"
    ```
...
??? "Example Output"
    ...
    - Generated unit tests for the `vulcan_example.full_model` model by live querying the data.
    ...
    Manually execute tests with `vulcan test`:
...
Pro tip: run this after `vulcan table_diff` to get a full picture of your changes.

=== "Vulcan"

    ```bash
    vulcan fetchdf "select * from vulcan_example__dev.full_model limit 5"
    ```
...
=== "Tobiko Cloud"
    ...
    ```bash
    tcloud vulcan fetchdf "select * from vulcan_example__dev.full_model limit 5"
    ```
...
2. Run Vulcan in verbose mode so you can verify its behavior.
...
=== "Vulcan"
    ...
    ```bash
    vulcan render vulcan_example.incremental_model
    ```

    ```bash
    vulcan render vulcan_example.incremental_model --dialect databricks
    ```

    ```bash
    vulcan render <model_name> --dialect <dialect>
    ```

=== "Tobiko Cloud"

    ```bash
    tcloud vulcan render vulcan_example.incremental_model
    ```

    ```bash
    tcloud vulcan render vulcan_example.incremental_model --dialect databricks
    ```

    ```bash
    tcloud vulcan render <model_name> --dialect <dialect>
    ```
...
??? "Example Output"
    ...
    Vulcan returns the full SQL code in the default or target dialect.
...
Verbose mode lets you see detailed operations in the physical and virtual layers. This is useful to see exactly what Vulcan is doing every step. After, you can copy/paste the fully qualified table/view name into your query console to validate the data (if that's your preference).
...
=== "Vulcan"
    ...
    ```bash
    vulcan plan <environment> -vv
    ```

=== "Tobiko Cloud"

    ```bash
    tcloud vulcan plan dev -vv
    ```

    ```bash
    tcloud vulcan plan <environment> -vv
    ```
...
Each time you perform a Vulcan command, it creates a log file in the `logs` directory. You can view them by manually navigating to the correct file name with latest timestamp or with this simple shell command.
...
??? "Example Output"
    ...
    This is the log file for the `vulcan plan dev` command. If you want to see the log file directly, you can click on the file path in the output to open it in your code editor.
...
Vulcan schedules your transformation on a per-model basis in proper DAG order. This makes it easy to configure how often each step in your pipeline runs to backfill data.

Vulcan won't schedule models whose upstream models are late or failed, and they will rerun from point of failure by default!
...
Note: `pending` and `late` states are only supported in Tobiko Cloud. In Vulcan, it will only understand if the model is ready or not ready to execute without mention of these states.

If you're using open source Vulcan, you can run this command in your orchestrator (ex: Dagster, GitHub Actions, etc.) every 5 minutes or at your lowest model cron schedule (ex: every 1 hour). Don't worry! It will only run executions that need to be run.
...
=== "Vulcan"
    ...
    ```bash
    vulcan plan <environment> --forward-only
    ```

=== "Tobiko Cloud"

    ```bash
    tcloud vulcan plan dev --forward-only
    ```

    ```bash
    tcloud vulcan plan <environment> --forward-only
    ```
...
If you notice you have a lot of old development schemas/data, you can clean them up with the following command. This process runs automatically during the `vulcan run` command. This defaults to deleting data older than 7 days.
