...
**Note:** The selector syntax described below is also used for the Vulcan `plan` [`--allow-destructive-model` and `--allow-additive-model` selectors](guides/plan.md#destructive-changes).

---

## Background

A Vulcan [plan](guides/plan.md) automatically detects changes between your local project and the deployed environment. When applied, it backfills directly modified models and their downstream dependencies.
...
- Learn about [Plans](guides/plan.md) for understanding plan behavior
- Read the [Plan Guide](guides/plan.md) for applying changes
- Check [Model Configuration](../configurations-old/configuration.md) for model properties
