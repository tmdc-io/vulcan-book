...
2. To preview an example of what your change looks like without actually creating a table, use the `vulcan evaluate` command. Refer to [evaluating a model](#evaluating-a-model) below.
3. To materialize this change, use the `vulcan plan` command. Refer to [previewing changes using the `plan` command](#previewing-changes-using-the-plan-command) below.

### Evaluating a model

The `evaluate` command will run a query against your database or engine and return a dataframe. It is used to test or iterate on models without database side effects and at minimal cost because Vulcan isn't materializing any data.
...
2. When you run the `evaluate` command, Vulcan detects the changes made to the model, executes the model as a query using the options passed to `evaluate`, and shows the output returned by the model query.

### Previewing changes using the `plan` command

When Vulcan runs the `plan` command on your environment, it will show you whether any downstream models are impacted by your changes. If so, Vulcan will prompt you to classify the changes as [Breaking](../concepts/plans.md#breaking-change) or [Non-Breaking](../concepts/plans.md#non-breaking-change) before applying the changes.

To preview changes using `plan`:

1. Enter the `vulcan plan <environment name>` command.
...
Before trying to revert a change, ensure that you have already made a change and that you have run the `vulcan plan` command.
...
**Note:** The Vulcan janitor runs periodically and automatically to clean up Vulcan artifacts no longer being used, and determines the time-to-live (TTL) for tables (how much time can pass before reverting is no longer possible).

## Validating changes to a model

### Automatic model validation

Vulcan automatically validates your models in order to ensure the quality and accuracy of your data. This is done via the following:
...
Vulcan also provides automatic validation via CI/CD by automatically creating a preview environment.
...
Before deleting a model, ensure that you have already run `vulcan plan`.
...
3. Plan and apply your changes to production, and enter `y` for the Virtual Update. By default, the `vulcan plan` command targets your production environment:
