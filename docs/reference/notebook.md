...
Vulcan supports Jupyter and Databricks Notebooks. Magics are loaded automatically when `vulcan` or one of its modules is imported.
...
## Vulcan project setup

Notebooks locate a Vulcan project by setting a `context` with either the Python API or a notebook magic.
...
```python
from vulcan.cli.project_init import init_example_project
